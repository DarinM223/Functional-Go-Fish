...
class PlayerSpec extends FlatSpec with Matchers {
  "PersonPlayer's hasCard" should "properly check if a player has a card" in {
    val player = PersonPlayer("Test", List(Card(1, Suite.Hearts), Card(2, Suite.Spades), Card(3, Suite.Diamonds)), 0)
  ...
  "PersonPlayer's addCard" should "properly add card to hand" in {
    val player = PersonPlayer("Test", List(Card(1, Suite.Hearts), Card(2, Suite.Spades)), 0)
    val discardPile = Map[Int, Boolean](4 -> true)
    val (newPlayer, newDiscardPile) = player.addCard(Card(3, Suite.Diamonds), discardPile)

    newPlayer.piles should be (0)
    newPlayer.cards.length should be (3)
    newPlayer.cards.contains(Card(1, Suite.Hearts)) should be (true)
    newPlayer.cards.contains(Card(2, Suite.Spades)) should be (true)
    newPlayer.cards.contains(Card(3, Suite.Diamonds)) should be (true)

    newDiscardPile should be (discardPile)
  }

  it should "remove books when you add a card" in {
    val player = PersonPlayer("Test", List(Card(1, Suite.Hearts), Card(1, Suite.Spades), Card(1, Suite.Diamonds)), 0)
    val discardPile = Map[Int, Boolean](4 -> true)
    val (newPlayer, newDiscardPile) = player.addCard(Card(1, Suite.Clubs), discardPile)
  ...
  "PersonPlayer's removeCard" should "properly remove card from hand" in {
    val player = PersonPlayer("Sample", List(Card(1, Suite.Hearts), Card(2, Suite.Spades), Card(3, Suite.Diamonds)), 0)

    var tuple = player.removeCard(1)
    var newPlayer = tuple._1
    var removedCards = tuple._2
    newPlayer.cards should be (List(Card(2, Suite.Spades), Card(3, Suite.Diamonds)))
    removedCards.length should be (1)

    tuple = newPlayer.removeCard(2)
    newPlayer = tuple._1
    removedCards = tuple._2
    newPlayer.cards should be (List(Card(3, Suite.Diamonds)))
    removedCards.length should be (1)
  ...
  it should "remove all cards of the rank" in {
    val player = PersonPlayer("Sample", List(Card(1, Suite.Hearts), Card(1, Suite.Spades), Card(1, Suite.Diamonds)), 0)
    val (newPlayer, removedCards) = player.removeCard(1)
    newPlayer.cards.isEmpty should be (true)
    removedCards.length should be (3)
    removedCards.contains(Card(1, Suite.Hearts)) should be (true)
    removedCards.contains(Card(1, Suite.Spades)) should be (true)
    removedCards.contains(Card(1, Suite.Diamonds)) should be (true)
  }

  "PersonPlayer's query" should "get another player's card if they have a card of same rank" in {
    val player = PersonPlayer("Sample", List(Card(1, Suite.Hearts), Card(2, Suite.Spades), Card(3, Suite.Diamonds)), 0)
    val player2 = PersonPlayer("Sample2", List(Card(4, Suite.Clubs), Card(7, Suite.Diamonds), Card(9, Suite.Spades)), 0)
  ...
  it should "draw from deck if other person doesn't have the card" in {
    val player = PersonPlayer("Sample", List(Card(1, Suite.Hearts), Card(2, Suite.Spades), Card(3, Suite.Diamonds)), 0)
    val player2 = PersonPlayer("Sample2", List(Card(2, Suite.Diamonds)), 0)
    val deck = CardUtils.standardDeck()
    val discardPile = Map[Int, Boolean](10 -> true)
    ...
    player.query(7, player2, deck, discardPile) match {
      case Player.QueryResult(player1, player2, deck, discardPile, success, ranOut) => {
        ...
        player1.cards.contains(Card(2, Suite.Spades)) should be (true)
        player1.cards.contains(Card(3, Suite.Diamonds)) should be (true)
        player2.cards should be (List(Card(2, Suite.Diamonds)))
